...
export const cache = {
  ...
  byParser: (parser: AccountParser) => {
    const result: string[] = [];
    keyToAccountParser.forEach((value: AccountParser, key: string) => {
        if (keyToAccountParser.get(key) === parser) {
          result.push(key);
        }
    });
...
export const getCachedAccount = (
  ...
  genericCache.forEach((account: ParsedAccountBase, key: string) => {
